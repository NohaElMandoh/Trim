...
@section('title', __('Editing').' '.ucfirst(__('address')))
@section('content')
<form role="form" class="form-horizontal" method="POST" action="{{ route('addresses.update', $row->id) }}">
    @csrf
    @method('PUT')
    <div class="form-body">
        @component('input_trans', ['type' => 'text', 'label' => 'Address', 'required' => true, 'model' => $row])
            address
        @endcomponent
        @component('input', ['type' => 'number', 'label' => 'Order', 'required' => true, 'value' => $row->order])
            order
        @endcomponent
