...
@section('title', __('Viewing').' '.ucfirst(__('socials')))
@section('content')
<div class="row">
        <div class="box">
            <div class="box-header">
            @can('social.create')
                    <a href="{{ route('socials.create') }}"  class="btn btn-primary"> {{ __('Add New')}}
                ...
                <table class="table table-striped table-bordered" id="example1">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td> {{ $row->id }} </td>
                            <td> {{ $row->url }} </td>
                            <td> <img src="{{ route('file_show', $row->image) }}" /> </td>
                            ...
                            <td>
                                <div class="btn-group">
                                    ...
                                    <ul class="dropdown-menu pull-left" role="menu">
                                        @can('social.view')
                                        <li>
                                            <a href="{{ route('socials.show', $row->id) }}">
                                                <i class="fa fa-eye"></i> {{ __('View') }} </a>
                                        </li>
                                        @endcan
                                        @can('social.edit')
                                        <li>
                                            <a href="{{ route('socials.edit', $row->id) }}">
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <form id="delete_form" method="POST" action="{{ route('socials.destroy', 0) }}">
                    @csrf
                    @method('DELETE')
