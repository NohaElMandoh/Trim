...
@section('title', __('Viewing').' '.ucfirst(__('More way orders')))
@section('content')
<div class="row">
    <div class="box">
        <div class="box-header">
            @can('moreway_order.create')
                <a href="{{ route('moreway_orders.create') }}"  class="btn btn-primary"> {{ __('Add New')}}
                        ...
                        <td>
                            <div class="btn-group">
                                ...
                                <ul class="dropdown-menu pull-left" order="menu">
                                    ...
                                    <li>
                                        <a href="{{ route('moreway_orders.show', $row->id) }}">
                                            <i class="fa fa-eye"></i> {{ __('View') }} </a>
                                    </li>
                                    @endcan
                                    @can('moreway_order.view')
                                    <li>
                                        <a href="{{ route('moreway_orders.messages', $row->id) }}">
                                            <i class="fa fa-eye"></i> {{ __('View messages') }} </a>
                                    </li>
                                    @endcan
                                    @can('moreway_order.status')
                                    <li>
                                        <a href="{{ route('moreway_orders.status', $row->id) }}">
                                            <i class="fa fa-pencil-square-o"></i> {{ __('Change status') }} </a>
                                    </li>
                                    @endcan
                                    @can('moreway_order.delete')
                                    <li>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <form id="delete_form" method="POST" action="{{ route('moreway_orders.destroy', 0) }}">
                    @csrf
                    @method('DELETE')
