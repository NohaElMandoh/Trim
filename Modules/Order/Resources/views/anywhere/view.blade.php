...
@section('title', __('Viewing').' '.ucfirst(__('Any where')))
@section('content')
<div class="panel panel-default">
    <div class="panel-heading">{{ __('Shop name') }}</div>
    <div class="panel-body">{{ $row->shop_name ?? '' }}</div>
</div>
<div class="panel panel-default">
...
<div class="panel panel-default">
    ...
    <div class="panel-body">
        @foreach ($row->items()->get() as $product)
            <div class="col-md-3">
                {{ __('Name') }} : {{ $product->name }} <br />
                {{ __('Quantity') }} : {{ $product->qty }} <br />
            </div>
        @endforeach
    </div>
</div>
