...
@section('title', __('Viewing').' '.ucfirst(__('OQ')))
@section('content')
...
<div class="panel panel-default">
    <div class="panel-heading">{{ __('Shop rate') }}</div>
    <div class="panel-body">
        {{ __('Rate') }} : {{ $row->shop_rate }} <br />
        {{ __('Review') }} : {{ $row->shop_review }} <br />
        @if($row->shop_review_image)
        {{ __('Image') }} : <img src="{{ route('file_show', $row->shop_review_image) }}" style="height: 60px" /> <br />
        @endif
    </div>
</div>
<div class="panel panel-default">
    ...
    <div class="panel-body">
        @foreach ($row->offers()->get() as $offer)
            <div class="col-md-3">
                ...
                {{ __('Description') }} : {{ $offer->description }} <br />
...
var map1 = new google.maps.Map(document.getElementById('buy-canvas'), {
    center:{
        lat: {{ $row->branch()->first()->lat }},
        lng: {{ $row->branch()->first()->lng }}
    },
    zoom: 15
});
