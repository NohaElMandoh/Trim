...
class WeekController extends Controller
{
    ...
    {
        $rows = Order::where('type', 'week')->latest()->get();
        return view('order::week.index', compact('rows'));
    }
    ...
    {
        $row = Order::where('type', 'week')->where('id', $id)->firstOrFail();
        return view('order::week.view', compact('row'));
    }
    ...
    {
        $row = Order::where('type', 'week')->where('id', $id)->firstOrFail();
        return view('order::week.status', compact('row'));
    }
    ...
    {
        $row = Order::where('type', 'week')->where('id', $id)->firstOrFail();
        return view('order::week.messages', compact('row'));
    }
