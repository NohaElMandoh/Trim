...
@section('content')
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            ...
            <img src="@if (!empty($row->image)) {{ url($row->image) }} @else
                url('uploads/user.png') @endif" />
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            ...
            <img src="@if (!empty($row->cover)) {{ url($row->cover) }} @else
                url('uploads/user.png') @endif" />
            </div>
        </div>
    </div>
    <div class="panel panel-default">
        ...
        <div class="panel-body"><a href="{{ route('file_show', $row->commercial_register) }}"
                target="_blank">{{ $row->commercial_register }}</a></div>
    </div>
    <div class="panel panel-default">
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            ...
            @foreach ($row->subscription()->get() as $subscription)
            @if ($subscription->pivot->is_active == 1)
              <div class="row">
                    <div class="col-md-3">{{ __('Title') }}:  {{ $subscription->title }}</div>
                    <div class="col-md-3">{{ __('Price') }}: {{  $subscription->pivot->price  }}</div>
                    <div class="col-md-3">{{ __('from') }}: 
                     {{   date('d-m-Y', strtotime($subscription->pivot->from ))}}
                        </div>
                    <div class="col-md-3">{{ __('to') }}: 
                        {{   date('d-m-Y', strtotime($subscription->pivot->to ))}}
                        </div>
                </div>
