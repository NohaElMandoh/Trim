...
@section('content')
    <form role="form" class="form-horizontal" method="POST" action="{{ route('salons.update', $row->id) }}"
        ...
        <div class="form-body">
            @component('input', ['type' => 'text', 'label' => 'Name', 'required' => true, 'value' => $row->name])
                name
            @endcomponent
            @component('input', ['type' => 'textarea', 'label' => 'Description', 'required' => false, 'value' =>
                $row->description])
                description
            @endcomponent
        ...
        @if (!empty($row->cover))
            @component('input_image', ['width' => 200, 'height' => 200, 'label' => 'cover', 'src' => url($row->cover)])
                cover
            @endcomponent
        @else @component('input_image', ['width' => 200, 'height' => 200, 'label' => 'cover', 'src' =>
            url('uploads/user.png')])
            cover
        @endcomponent
    @endif
        ...
        <div class="col-md-10">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <a href="{{ route('file_show', $row->commercial_register) }}"
                        target="_blank">{{ $row->commercial_register }}</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="form-group form-md-line-input">
        ...
        <div class="col-md-10">
            <select class="js-example-basic-single js-states form-control" id="governorate_id"
                name="governorate_id">
                @foreach (\Modules\Governorate\Entities\Governorate::latest()->get() as $governorate)
                    <option value="{{ $governorate->id }}"
                        {{ old('governorate_id') == $governorate->id || $governorate->id == $row->governorate_id ? 'selected' : '' }}>
    ...
    <div class="form-group form-md-line-input">
        ...
        <div class="col-md-10">
            <select class="js-example-basic-single js-states form-control" id="gender" name="gender">
                <option value="male" {{ old('gender') == 'male' || $row->gender == 'male' ? 'selected' : '' }}>
                    {{ ucfirst(__('male')) }}</option>
                <option value="female"
                    {{ old('gender') == 'female' || $row->gender == 'female' ? 'selected' : '' }}>
                    {{ ucfirst(__('female')) }}</option>
            </select>
            <div class="form-control-focus"> </div>
        </div>
    </div>
...
@endforeach
    ...
    <div class="form-group form-md-line-input">
        ...
        <div class="col-md-10">
            <select class="js-example-basic-single js-states form-control" id="services[]" name="services[]"
                multiple="multiple">
                @foreach (\Modules\Service\Entities\Service::all() as $service)
                    <option value="{{ $service->id }}" @if (old('services'))  @if (in_array($service->id,
                        old('services'))) selected @endif @else @if (in_array($service->id, $selected)) selected @endif
