...
@section('title', __('Viewing').' '.ucfirst(__('delegates')))
@section('content')
<div class="row">
    <div class="box">
        <div class="box-header">
            @can('user.create')
                <a href="{{ route('delegates.create') }}"  class="btn btn-primary"> {{ __('Add New')}}
                        ...
                        <td>
                            <div class="btn-group">
                                ...
                                <ul class="dropdown-menu pull-left" user="menu">
                                    @can('user.view')
                                    <li>
                                        <a href="{{ route('delegates.show', $row->id) }}">
                                            <i class="fa fa-eye"></i> {{ __('View') }} </a>
                                    </li>
                                    @endcan
                                    @can('user.edit')
                                    <li>
                                        <a href="{{ route('delegates.edit', $row->id) }}">
                                            <i class="fa fa-pencil-square-o"></i> {{ __('Edit') }} </a>
                                    </li>
                                    @endcan
                                    @can('user.edit')
                                    <li>
                                        <a href="{{ route('delegates.password', $row->id) }}">
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <form id="delete_form" method="POST" action="{{ route('delegates.destroy', 0) }}">
                    @csrf
                    @method('DELETE')
